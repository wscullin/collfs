...
struct do_dlcollfs_comm_push_args
{
  /* Arguments to do_dlcomm_push.  */
  MPI_Comm comm;

  /* Return values of do_dlcomm_push.  */
  int _return;
};

struct do_dlcollfs_init_pointers_args
{
  /* Arguments to do_dlcollfs_init_pointers.  */
...
static void
do_dlcollfs_comm_push (void *ptr)
{
  struct do_dlcollfs_comm_push_args *args = (struct do_dlcollfs_comm_push_args *) ptr;
  args->_return = GLRO(_collfs_comm_push)(args->comm));  
}

static void
do_dlcollfs_init_pointers (void *ptr)
{
  struct do_dlcollfs_init_pointers_args *args = (struct do_dlcollfs_init_pointers_args *) ptr;
...
static struct dl_open_hook _dl_open_hook =
  {
    ...
    .dlcollfs_comm_push = __libc_collfs_comm_push
    .dlcollfs_init_pointers = __libc_collfs_init_pointers
  };
#endif
...
__libc_dlopen_mode (const char *name, int mode)
{
  ...
  __libc_register_dlcollfs_hook (args.map);
...
int __libc_collfs_comm_push(MPI_Comm comm)
{
#ifdef SHARED
  if (__builtin_expect (_dl_open_hook != NULL, 0))
    return _dl_open_hook->dlcollfs_comm_push(comm);
#endif
  return (dlerror_run(do_dlcollfs_comm_push, &args) ? -1 : args._return);
}
libc_hidden_def (__libc_collfs_comm_push)

void __libc_collfs_init_pointers(int (*_MPI_Initialized)(int *flag),
                           int (*_MPI_Comm_rank) (MPI_Comm comm, int *rank),
                           int (*_MPI_Bcast) (void *buffer, int count, 
                                             MPI_Datatype datatype, int root,MPI_Comm comm),
                           int (*_MPI_Allreduce) (void *sendbuf, void *recvbuf, int count,
                           ...
                           int (*_MPI_Barrier) (MPI_Comm comm))
{
  struct do_dlcollfs_init_pointers_args args;
...
#ifdef SHARED
  if (__builtin_expect (_dl_open_hook != NULL, 0))
    return _dl_open_hook->dlcollfs_init_pointers(_MPI_Initialized,
                                                 _MPI_Comm_rank,
                                                 _MPI_Bcast,
                                                 _MPI_Allreduce,
                                                 _MPI_Barrier);
  ...
  return dlerror_run(do_dlcollfs_init_pointers, &args);
}
libc_hidden_def (__libc_collfs_init_pointers)
