			...
			int mode, void **stack_endp, Lmid_t nsid)
{
  ...
  if (__builtin_expect (ICEPT_FXSTAT64 (_STAT_VER, fd, &st) < 0, 0))
      ...
      if (make_consistent)
	{
 	...
 	/* This looks like an error case, and this debug event almost never occurs. */
	/* Not sure if we want to optimize???  Leave it unoptimized unless it is a problem. */

          r->r_state = RT_CONSISTENT;
          if (GLRO(dl_debug_mask) & DL_DEBUG_EVENTS) _dl_debug_printf("\ndebug event: CONSISTENT\n"); 
	  _dl_debug_state ();
...
# define MAP_ANON 0
  ...
  if (r->r_state == RT_CONSISTENT)
    ...
    if (!(mode & __RTLD_SPROF)) assert (r->r_state == RT_ADD);
